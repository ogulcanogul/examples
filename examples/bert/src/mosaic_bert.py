...
from examples.bert.src.bert_layers import (BertForMaskedLM,
                                           BertForSequenceClassification, BertForSemanticSearchModel)
...
def create_mosaic_bert_semantic_search(pretrained_model_name: str = 'bert-base-uncased',
                           model_config: Optional[dict] = None,
                           tokenizer_name: Optional[str] = None,
                           gradient_checkpointing: Optional[bool] = False,
                           pretrained_checkpoint: Optional[str] = None):
    ...
    if pretrained_checkpoint is not None:
        model = BertForSemanticSearchModel.from_composer(
            pretrained_checkpoint=pretrained_checkpoint, config=config)
    else:
        model = BertForSemanticSearchModel(config)
