...
from examples.common.builders import (build_algorithm, build_callback,
                                      build_dataloader, build_dataloader_semantic_search, build_logger,
...
def build_model(cfg: DictConfig):
    if cfg.name == 'hf_bert':
        ...
        return create_mosaic_bert_mlm(
            ...
            tokenizer_name=cfg.get('tokenizer_name', None),
            gradient_checkpointing=cfg.get('gradient_checkpointing', None))
    elif cfg.name == 'mosaic_bert_semantic_search':
        return create_mosaic_bert_semantic_search(
            pretrained_model_name=cfg.pretrained_model_name,
            pretrained_checkpoint=cfg.get('pretrained_checkpoint', None),
            model_config=cfg.get('model_config', None),
            tokenizer_name=cfg.get('tokenizer_name', None),
...
def main(cfg: DictConfig,
    ...
    train_loader = build_dataloader_semantic_search(cfg.train_loader,
                                    cfg.device_train_batch_size)
    print('Building eval loader...')
    eval_loader = build_dataloader_semantic_search(cfg.eval_loader, cfg.device_eval_batch_size)
