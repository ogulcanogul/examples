...
class StreamingTextDatasetSemanticSearch(StreamingDataset):
    """Generic text dataset using MosaicML's StreamingDataset.
    ...
    def _tokenize(self, text_sample):
        ...
        query_tokens = self.tokenizer(text_sample['query'],
                              truncation=True,
                              padding='max_length',
                              max_length=self.max_seq_len)
        positive_tokens = self.tokenizer(text_sample['positive'],
                              truncation=True,
                              padding='max_length',
                              max_length=self.max_seq_len)
        negative_tokens = self.tokenizer(text_sample['negative'],
                              truncation=True,
                              padding='max_length',
                              max_length=self.max_seq_len)


        return [query_tokens, positive_tokens, negative_tokens]
...
def build_text_dataloader_for_semantic_search(cfg: DictConfig, device_batch_size: int):
    assert cfg.name == 'text', f'Tried to build text dataloader with cfg.name={cfg.name}'
    if cfg.dataset.get('group_method', None) is not None:
        raise NotImplementedError(
            'group_method is deprecated and has been removed.\nTo ' +
    ...
    # build dataset potentially with streams
    dataset = StreamingTextDatasetSemanticSearch(
        tokenizer_name=cfg.dataset.tokenizer_name,
        max_seq_len=cfg.dataset.max_seq_len,
        streams=streams,
        remote=cfg.dataset.get('remote', None),
